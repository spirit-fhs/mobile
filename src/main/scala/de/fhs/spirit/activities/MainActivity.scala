...
import de.fhs.spirit.tasks.NewsLoadTask
import de.fhs.spirit.toilers.{ViewBuilder, SpiritConnect, SpiritHelpers}
...
class MainActivity extends Activity {
  ...
  def menuHandler(context: Context, functionName: String) {
    functionName match {
      case "readNews" => /*if (SpiritConnect.checkInternetConnection(context)) {
        new NewsLoadTask(MainActivity.this).execute(null)
      } else {      */
